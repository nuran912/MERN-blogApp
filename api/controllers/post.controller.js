...
export const deletepost = async ( req,res, next) => {
    if(!req.user.isAdmin || req.user.id !== req.params.userId){    //first we need to check if the user is an Admin or the owner of the post
...
export const updatePost = async (req,res,next) => {
    ...
    try {
        const updatedPost = await Post.findByIdAndUpdate(   //we pass the id of the post and the values we wanna change
            req.params.postId,  //
            {
                $set: {
